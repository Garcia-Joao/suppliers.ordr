      ...
      <div className="space-y-4 pb-24 lg:pb-0">
        <section className="relative overflow-hidden rounded-[2rem] border bg-card p-5 shadow-2xl shadow-black/5 md:p-8">
          ...
          <div className="relative grid gap-6 xl:grid-cols-[1.3fr_0.7fr] xl:items-stretch">
            ...
            <StatusPanel profile={data?.supplier ?? null} loading={loading} />
          </div>
        </section>
        ...
        {data && (
          <>
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <MetricCard icon={CheckCircle2} label="Produtos vinculados" value={data.linkedProducts} />
...
function StatusPanel({ profile, loading }: { profile: SupplierProfile | null; loading: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 space-y-3">
        <StatusLine label="Disponibilidade manual" value={profile?.onlineEnabled ? 'Ligada' : 'Pausada'} />
        <StatusLine label="Dentro do expediente" value={status?.insideOperatingHours ? 'Sim' : 'Não'} />
        <StatusLine label="Código ORDR" value={profile?.ordrCode || '-'} />
      </div>
    </motion.article>
  )
}
