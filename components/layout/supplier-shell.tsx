      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 lg:grid-cols-[280px_1fr]">
        <aside className="glass-card sticky top-5 hidden h-[calc(100vh-2.5rem)] rounded-[2rem] p-4 lg:block">
          <div className="mb-8 flex items-center justify-between gap-3 px-2 pt-2">
            ...
            <button
              onClick={toggleTheme}
              className="grid size-10 place-items-center rounded-2xl border bg-background/60 text-muted-foreground hover:text-foreground"
          ...
          <div className="absolute inset-x-4 bottom-4 rounded-3xl border bg-background/60 p-4">
            ...
            <button
              onClick={logout}
              className="flex w-full items-center justify-center gap-2 rounded-2xl border px-3 py-2 text-sm font-bold hover:bg-secondary"
            >
        ...
        <main className="min-w-0">
          <header className="glass-card sticky top-3 z-20 mb-4 flex items-center justify-between rounded-[1.75rem] px-4 py-3 lg:hidden">
            ...
            <div className="flex items-center gap-2">
              <button onClick={toggleTheme} className="rounded-2xl border px-3 py-2 text-sm font-bold">
                {theme === 'dark' ? 'Claro' : 'Escuro'}
              </button>
              <button onClick={logout} className="rounded-2xl border px-3 py-2 text-sm font-bold">
                Sair
              </button>
            </div>
          </header>
            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={clsx(
                    'grid place-items-center rounded-2xl p-2 text-xs font-bold',
